...
class InternalRealInterval(UniqueRepresentation, Parent):
    ...
    def left_point_with_epsilon(self, closure=False):
        ...
        if self.is_empty():
            ...
            return self._lower, 0 if closure else 1
    ...
    def right_point_with_epsilon(self, closure=False):
        ...
        if self.is_empty():
            ...
            return self._upper, 1 if closure else 0

    def interval_contained_in_interval(self, other):
        Int_1 = (self.left_point_with_epsilon(self), self.right_point_with_epsilon(self))
        Int_2 = (other.left_point_with_epsilon(other), other.right_point_with_epsilon(other))
        return Int_2[0] <= Int_1[0] and Int_1[1] <= Int_2[1]
...
class RealSet(UniqueRepresentation, Parent, Set_base,
    ...
    @staticmethod
    def intersection_of_intervals(interval_lists):
        ...
        EXAMPLES::
            ...
            [2, 3]
        """
        # scan = merge(*[RealSet(interval_list).scan_coho_interval_list(tag=index) for index, interval_list in
        #                enumerate(interval_lists)])
        scan = []
        intersection = []
        for index, interval_list in enumerate(interval_lists):
            for interval in interval_list:
                real_set = RealSet(interval)
    ...
    @staticmethod
    def scan_union_of_intervals_minus_union_of_intervals(interval_lists, remove_lists, remove_closure=False):
        # Following uses the lexicographic comparison of the tuples.
        scan = []
        for interval_list in interval_lists:
            for interval in interval_list:
                real_set = RealSet(interval)
                scan.append(real_set.scan_interval(True))
        for remove_list in remove_lists:
            for remove in remove_list:
                real_set_remove = RealSet(remove)
                scan.append(real_set_remove.scan_interval(False))
        scan = merge(*scan)
    ...
    @staticmethod
    def interval_list_from_scan(scan, old_fashioned_closed_intervals=False):
    ...
    @staticmethod
    def union_of_intervals_minus_union_of_intervals(interval_lists, remove_lists,
                                                              old_fashioned_closed_intervals=False,
                                                              remove_closure=False):
        ...
        EXAMPLES::
            sage: RealSet.union_of_intervals_minus_union_of_intervals([[[0,10]]], [[[2,2], [3,4]]])
            ([0, 2), (2, 3), (4, 10])
            sage: RealSet.union_of_intervals_minus_union_of_intervals([[[0, 10]]], [[[1, 7]], [[2, 5]]])
            ([0, 1), (7, 10])
            sage: RealSet.union_of_intervals_minus_union_of_intervals([[[0, 10], RealSet.closed_open(10,20)]],[ ])
            ([0, 20),)
            sage: RealSet.union_of_intervals_minus_union_of_intervals([[[0, 10], RealSet.closed_open(10,20)]],[ ], old_fashioned_closed_intervals=True)
             ([0, 20),)
        """
        for interval_list in interval_lists:
            for interval in interval_list:
                real_set = RealSet(interval)
        gen = real_set.interval_list_from_scan(
            real_set.scan_union_of_intervals_minus_union_of_intervals(interval_lists, remove_lists,
                                                                                remove_closure),
